...
**Status: ALL 7 batches MERGED + DEPLOYED; migration `0077` applied to prod D1.** `main` HEAD **`4fbbf4a`** (Batch 7 docs-sync merge; batches 1–6 code landed at `69e6a74`), **3149 / 276** tests, migrations **0001–0077**. The feature is **dormant** in prod until the first review (~2026-07-25 — the invite cron's `REVIEW_LAUNCH_CUTOFF_MS`=2026-06-28 + the 18–48h post-event window mean the first invites go out the night after **Operation Last Light** ends, 25 July 2026). Until then there is **no `aggregateRating` anywhere and no star rating on the homepage** — the honest interim (the fabricated 4.9★ is gone; operator-confirmed). **Operator-pending:** (1) decide the **CAN-SPAM classification** of the `review_invite` email before the first real send (~2026-07-27) — see the runbook; (2) after the Batch-6 deploy settles, **recapture the `home` public visual baseline** (dropping the 4th hero "Avg. Rating" stat drifts `home.png`; the public visual suite tests LIVE prod).
