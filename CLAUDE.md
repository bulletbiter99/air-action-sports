...
- **Deliberately DECLINED** (operator-agreed, not oversights): `AdminCampaigns` FilterBar (kept its segmented status-button row — the chip "+ Add filter" flow is more clicks for a single filter), `AdminDashboardPersona` header (personalized landing-dashboard header), FieldRentals detail/new page primary buttons (still rounded).
