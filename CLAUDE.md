...
### Milestone 4 — Bookings + Detail Workspace + New Admin Shell (in progress; B0-B7 shipped 2026-05-07)
...
| **B12a** Legacy dispatcher + Legacy AdminDashboard removal (this batch) | Pure dead-code deletion. `src/admin/AdminDashboard.jsx` collapsed from 671 lines to 6 (re-exports `AdminDashboardPersona` directly). `src/admin/AdminLayout.jsx` removes `NAV_SECTIONS` constant + the `useFeatureFlag('new_admin_dashboard')` dispatcher branch in `Sidebar` + the `{newAdminDashboard && <CheckInBanner />}` gate (now unconditional render). The new sidebar config (`sidebarConfig.js`) becomes the sole production path. Tests unchanged (918/100; legacy code wasn't reachable). Lint warnings DROP from 293 → 281 (-12; removed JSX-component false-positives for deleted helpers `CronHealth`, `StatusBadge`, `MethodBadge`, `BookingDetailModal`, `AttendeeRow`, `RefundConfirmModal`, `Row`, `StatCard`). Build bundle DROPS by ~23 kB (the legacy `AdminDashboard-*.js` chunk gone; AdminDashboard now IS the persona shell). Remaining `useFeatureFlag` calls for the 3 M4 flags untouched (B12b cleanup). DB unchanged (B12b/c handle flag-row DELETE). | pending | pending |
| **B12b** Flag-check removal + flag-row DELETE | Remove the now-redundant `useFeatureFlag(...)` calls for `command_palette` (1 site in AdminLayout.jsx), `customers_entity` (4 sites: AdminLayout.jsx Sidebar, AdminCustomers.jsx, AdminCustomerDetail.jsx, CommandPalette.jsx via `commandsFromSidebar`, sidebarConfig.js `requiresFlag`), and any remaining `new_admin_dashboard` (CheckInBanner.jsx, CustomerTypeahead.jsx, AdminNewBooking.jsx). Code change ships first; operator runs `DELETE FROM feature_flags WHERE key IN ('command_palette', 'customers_entity', 'new_admin_dashboard');` AFTER deploy. Rough estimate: 5-7 source files + CLAUDE.md + HANDOFF.md. | pending | pending |
| **B12c** Closing — `/admin/today` page + runbooks | New `src/admin/AdminToday.jsx` (resurfaces dynamic Today nav item from B5 + Roster/Scan/Rentals as sub-actions per D09); route in `src/App.jsx`; sidebarConfig.js Today entry promoted from dynamic-placeholder to real link. 3 closing runbooks: `docs/runbooks/m4-baseline-coverage.txt`, `docs/runbooks/m4-deploy.md`, `docs/runbooks/m4-rollback.md`. Final M4 close. | pending | pending |

**Cumulative through B12a (current):** **918 unit tests across 100 files** (+301 vs M3 close; B8 + B9 docs-only; B12a deletes ~635 lines of legacy code with no test impact). 7 visual regression baselines captured and CI-gated at 1% threshold. **28 gated paths** in `scripts/test-gate-mapping.json` (M3's 12 + M4's 9 new + 7 carryover). **4 D1 migrations applied to remote in M4**: 0026 (saved_views), 0027 (bookings_refund_external + email template seed), 0028 (users.persona column + role-based backfill), 0029 (command_palette flag). **B8 + B9 in-place SQL flips on remote D1** (no migration files; mutations of seeded `feature_flags` rows): B8 → `command_palette` `off` → `on` and `customers_entity` `role_scoped='owner'` → `on`; B9 → `new_admin_dashboard` `role_scoped='owner'` → `on`. **All 3 M4 feature flags now `state='on'`** (live for all admins). 9 decision-register entries (D01-D09). Lint: 0 errors / **281 warnings** (down 12 from B9 baseline — B12a removed JSX-usage false-positives for deleted helpers `CronHealth`, `StatusBadge`, `MethodBadge`, `BookingDetailModal`, `AttendeeRow`, `RefundConfirmModal`, `Row`, `StatCard` plus various `Stat` / `StatusPill` / `Link` / `ActionQueueStat` overlap from removed legacy modal). **Bundle DROPS by ~23 kB** (the legacy `AdminDashboard-*.js` chunk gone; AdminDashboard.jsx now re-exports `AdminDashboardPersona` directly).
...
- **Roster/Scan/Rentals (D09)**: routes stay alive (deep-link works); sidebar hides them per D09 when `new_admin_dashboard='on'`. They resurface inside `/admin/today` when B12 ships that page. B4c's TodayCheckIns widget already deep-links to `/admin/scan?event=...`.
...
- **B5 sidebar + B6 walk-up speed wins + B7 Command Palette** all flag-gated — `new_admin_dashboard` for sidebar + walk-up; `command_palette` for Cmd+K. **As of B12a** (post-cleanup): `command_palette='on'`, `customers_entity='on'`, `new_admin_dashboard='on'` — all 3 M4 flags live for every admin. **B12a deleted** legacy `AdminDashboardLegacy()` + `NAV_SECTIONS` source code (was dead at runtime since B9; physically removed now). Remaining `useFeatureFlag` calls for the 3 M4 flags are now no-ops returning `true` — they get cleaned up in B12b along with `DELETE FROM feature_flags` for those rows. Pre-B8 production state had `customers_entity` and `new_admin_dashboard` both at `role_scoped='owner'` outside any documented batch — HANDOFF.md §12 was inaccurate; B8's docs PR corrected it.
...
2. `npm install`
3. `npm test` — confirm **918/100 passing**
4. `npm run lint` — confirm 0 errors / **281 warnings** (down 12 from B9 baseline; B12a removed JSX false-positives for deleted helpers)
5. `npm run build` — confirm clean (~250ms; bundle ~23 kB smaller post-B12a)
6. `curl https://airactionsport.com/api/health` — confirm `{"ok":true,...}`
7. **Pick the next batch:**
   - B12b = flag-check removal + flag-row DELETE. Remove the now-redundant `useFeatureFlag(...)` calls for the 3 M4 flags from 5-7 source files; operator runs `DELETE FROM feature_flags WHERE key IN ('command_palette', 'customers_entity', 'new_admin_dashboard');` AFTER deploy.
   - B12c = closing — `/admin/today` page activation + 3 closing runbooks.
   - B10 + B11 are collapsed/deferred (B10 = direct flip path; B11 = admin visual regression deferred to post-M4).
   - Or pick from the §11 pre-launch operational checklist in HANDOFF.md (DMARC/DKIM, Stripe sandbox→live cutover, etc.).
