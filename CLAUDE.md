...
**Cumulative through B12b (current):** **917 unit tests across 100 files** (+300 vs M3 close; -1 vs B12a baseline — B12b removed 2 redundant flag-off tests + modified 2 + added 1 forward-compat synthetic test). 7 visual regression baselines captured and CI-gated at 1% threshold. **28 gated paths** in `scripts/test-gate-mapping.json` (M3's 12 + M4's 9 new + 7 carryover). **4 D1 migrations applied to remote in M4**: 0026 (saved_views), 0027 (bookings_refund_external + email template seed), 0028 (users.persona column + role-based backfill), 0029 (command_palette flag). **B8 + B9 in-place SQL flips on remote D1** (no migration files; mutations of seeded `feature_flags` rows): B8 → `command_palette` `off` → `on` and `customers_entity` `role_scoped='owner'` → `on`; B9 → `new_admin_dashboard` `role_scoped='owner'` → `on`. **All 3 M4 flag rows pending operator-driven DELETE** post-B12b deploy. 9 decision-register entries (D01-D09). Lint: 0 errors / **281 warnings** (unchanged from B12a). **Bundle: AdminDashboard chunk 28.09 kB** (persona shell as the only path).
...
2. `npm install`
3. `npm test` — confirm **917/100 passing** (down 1 from B12a; B12b cleanup of redundant flag-off tests)
4. `npm run lint` — confirm 0 errors / **281 warnings** (unchanged from B12a)
5. `npm run build` — confirm clean (~250ms)
6. `curl https://airactionsport.com/api/health` — confirm `{"ok":true,...}`
7. **Pick the next batch:**
   - B12c = closing — `/admin/today` page activation + 3 closing runbooks. Final M4 close.
   - **Operator action queued post-B12b deploy**: run `DELETE FROM feature_flags WHERE key IN ('command_palette', 'customers_entity', 'new_admin_dashboard');` to remove orphan rows (no consumer code remains).
