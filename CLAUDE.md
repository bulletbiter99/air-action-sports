...
| **B8** Atomic ancillary flag rollout (this batch) | SQL UPDATEs only — `command_palette` → `state='on'`, `customers_entity` → `state='on'` on remote D1. **Pre-flip discovery**: `customers_entity` + `new_admin_dashboard` were already at `role_scoped='owner'` outside any documented batch (updated_at ~16h before B8). For the current admin set (4 users, all `role='owner'`), `role_scoped='owner'` was already effectively `on`; B8 formalizes `customers_entity` and opens it to any future non-owner admin. `command_palette` flipped from `off` to `on` (true new behavior — Cmd+K live for all admins). `new_admin_dashboard` left at `role_scoped='owner'` for B9 to resolve. Docs PR only; tests/lint/build unchanged (918/100, 0 errors / 293 warnings, ~232ms). | pending | pending |
| **B9 / B10** Continue flag rollout | SQL UPDATEs only. `new_admin_dashboard` is currently at `role_scoped='owner'` (effectively dogfooded by all 4 current admins). B9 candidates: (a) flip directly to `on` to formalize for future non-owner admins, or (b) pass through `user_opt_in` first if a dogfood pause is preferred — operator decides. B10: final flip if B9 took the user_opt_in detour; otherwise collapse into B11/B12. Legacy `AdminDashboardLegacy()` + `NAV_SECTIONS` removal moves to B12 per D04 (not B10 as the previous summary suggested). Flag rows DELETEd in B12. | pending | pending |
| **B11** Coverage hardening | Likely admin visual regression baselines (persona dashboard + new sidebar — 4 admin pages noted in B5 audit) + edge-case backfill on uncovered surfaces. | pending | pending |
| **B12** Closing | Runbooks (m4-baseline-coverage / m4-deploy / m4-rollback), `/admin/today` page activation (resurfaces dynamic Today nav item from B5 + Roster/Scan/Rentals as sub-actions), legacy `AdminDashboardLegacy()` + `NAV_SECTIONS` removal (per D04), flag-row deletion. | pending | pending |

**Cumulative through B8 (current):** **918 unit tests across 100 files** (+301 vs M3 close; +162 vs M4 B3b mid-point; B8 was docs-only — no test/lint/build delta). 7 visual regression baselines captured and CI-gated at 1% threshold. **28 gated paths** in `scripts/test-gate-mapping.json` (M3's 12 + M4's 9 new + 7 carryover). **4 D1 migrations applied to remote in M4**: 0026 (saved_views), 0027 (bookings_refund_external + email template seed), 0028 (users.persona column + role-based backfill), 0029 (command_palette flag). **B8 in-place SQL flips on remote D1** (no migration file; mutations of seeded rows): `command_palette` `off` → `on`; `customers_entity` `role_scoped='owner'` → `on`. 9 decision-register entries (D01-D09). Lint: 0 errors / 293 warnings (273 M3 baseline + 20 false-positive JSX-usage warnings accumulated through M4 — all internal helper components — `Stat`, `StatusPill`, `Link`, `ActionQueueStat`, `CapacityBar`, `PendingStat`, `NewSidebarNav`, `SidebarItem`, `SidebarGroup`, `CheckInBanner`, `CustomerTypeahead`, `CommandPalette`, `BarChart`).
...
- **B5 sidebar + B6 walk-up speed wins + B7 Command Palette** all flag-gated — `new_admin_dashboard` for sidebar + walk-up; `command_palette` for Cmd+K. **As of B8**: `command_palette='on'` (Cmd+K live for all admins); `customers_entity='on'` (Customers nav entry visible to all admins, replacing the previously partial `role_scoped='owner'` state); `new_admin_dashboard` still at `role_scoped='owner'` (effectively visible to all 4 current owner-admins; B9/B10 will formalize). Pre-B8 production state had `customers_entity` and `new_admin_dashboard` both at `role_scoped='owner'` outside any documented batch — HANDOFF.md §12 was inaccurate; B8's docs PR corrects it.
...
4. `npm run lint` — confirm 0 errors / 293 warnings
5. `npm run build` — confirm clean (~250ms)
6. `curl https://airactionsport.com/api/health` — confirm `{"ok":true,...}`
7. **Pick the next batch:**
   - B9 = continue flag rollout. `new_admin_dashboard` is currently `role_scoped='owner'`. Decide: flip to `on` directly (formalizes for future non-owner admins) or pass through `user_opt_in` first (dogfood pause). SQL UPDATE only.
   - B10 = final flip if B9 took the `user_opt_in` detour; otherwise collapse into B11/B12.
   - B11 = coverage hardening (admin visual regression baselines).
   - B12 = closing (runbooks + /admin/today page activation + legacy `AdminDashboardLegacy()` + `NAV_SECTIONS` removal per D04 + flag-row deletion).
