...
| **B7c** Session handoff refresh (this batch) | HANDOFF.md §10/§12/§14 + prompt block refreshed for M4 B0-B7 state; CLAUDE.md M4 batch table refreshed with B4-B7 SHAs/PRs. No code changes; docs hygiene only. | pending | pending |
| **B8 / B9 / B10** Operator-paced flag rollouts | SQL UPDATEs only; no code beyond legacy AdminDashboardLegacy + NAV_SECTIONS removal in B10. (`new_admin_dashboard` → user_opt_in → on; `customers_entity` → on; `command_palette` → on; legacy code removed; flag rows DELETEd) | pending | pending |
...
- **B5 sidebar + B6 walk-up speed wins + B7 Command Palette** all flag-gated — `new_admin_dashboard` for sidebar + walk-up; `command_palette` for Cmd+K. Production flips happen in B8/B9/B10.
...
7. **Pick the next batch:**
   - B8/B9/B10 = operator-paced flag rollouts. Decide rollout cadence — all SQL UPDATE decisions; only B10 has code (legacy AdminDashboardLegacy + NAV_SECTIONS removal after `new_admin_dashboard='on'` is stable globally).
   - B11 = coverage hardening (admin visual regression baselines).
   - B12 = closing (runbooks + /admin/today page activation + final cleanup).
