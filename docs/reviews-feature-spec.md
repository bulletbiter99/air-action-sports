...
1. ✅ **RESOLVED (2026-07-01) — CAN-SPAM classification = TRANSACTIONAL + deliverability suppression (option B).** The invite is a one-per-booking, promotion-free message tied to a completed transaction → shipped without a postal-address/unsubscribe footer. As deliverability hygiene the sweep now skips addresses with a recorded hard bounce / spam complaint (`email_events.suppressed_marketing=1`, matched via `normalizeEmail` → `recipient_normalized`; best-effort — a check failure never blocks invites; suppressed candidates are NOT sentinel-stamped, so they age out of the 18–48h window naturally). Deliberately NOT gated on `customers.email_marketing` — that is a marketing preference, and opted-out customers still get their transactional invite.
