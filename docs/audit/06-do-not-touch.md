...
| `worker/lib/waiverLookup.js` `findExistingValidWaiver` ⚠️ **moved** — this row said `worker/routes/webhooks.js` until M2 B4a relocated the function (body byte-identical); `webhooks.js` only imports it now | shared with public site + handles waivers | Characterization test asserting: matches by `LOWER(TRIM(email))` + `LOWER(TRIM(player_name))`, requires `claim_period_expires_at > now`, returns null on missing inputs, returns latest by `signed_at` on multi-match. Required before any change because it ships from one file but is called from two flows (webhook + admin manual). |
...
| `worker/routes/bookings.js` `POST /api/bookings/checkout` | handles payments + customer-facing | Public checkout entry — every dollar enters here. Don't change the `pending_attendees_json` write shape (the Stripe webhook reads it back). Don't change the response shape — ⚠️ **it is `{ stripeUrl, bookingId }`** (`bookings.js:380`), NOT the `{ url, sessionId }` this row claimed until 2026-07-28. |
...
| `worker/lib/emailSender.js` (every existing named sender — **17 today**, not the "9" this row cited at Phase 1; the count keeps growing, so the operative rule is *never modify an existing sender; appending a new one is safe*) | customer-facing email | Every customer email passes through here. Modifying an existing sender's variable name, template slug, or tag shape ripples to webhook + admin + cron. New senders are safe. |
...
| `worker/lib/templates.js` (`loadTemplate`, `renderTemplate`) | customer-facing email | Variable interpolation rules. Breaking `{{var}}` syntax breaks every seeded template (16 at Phase 1; 34+ today — 19 migrations seed them). |
