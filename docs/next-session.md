...
| Open milestone | **None active.** The **accounting roadmap is FULLY COMPLETE** (11 surfaces) — the 2026-06-24 suite (#319–#328) plus 2026-06-25's **A/R aging + DSO** (#330), **Owner weekly scorecard** (#332), and **refund-side Stripe-fee reconciliation** (#335). **No roadmap items remain.** Remaining work is operator activation only (Marketing send + Resend webhook + FTS flag). **CI note:** the Admin visual regression check is **green** (#331 recaptured the baselines that had drifted from the accounting suite) — no more merging-through-red. |
