...
**⚠️ DISCOVERY (2026-07-01, during the post-publish baseline recapture):** the public visual suite's event-dependent baselines are **error-state captures and always have been** — the committed `events-listing` PNG (both the original M4 capture and the 2026-07-01 recapture) shows *"Couldn't load events. Please refresh in a moment."* because **`/api/events` has never successfully loaded from GitHub-runner CI** (cause TBD — likely Cloudflare bot-management challenging datacenter-IP XHR; the route has no rate limiter). The check stays green because every CI run reproduces the same error state — so the events grid/home-hero content has **zero real visual coverage**, and the earlier "CF edge-cache staleness" theory (memory `visual-baseline-cf-cache-gotcha`) was a misdiagnosis of this. Publishing Fire Storm therefore did NOT drift the baselines. **Fix directions:** route-mock the public suite's API calls with representative fixtures (the proven `tests/visual-admin` pattern — deterministic + actually covers event content), or diagnose/allowlist the CI→prod API block (operator's Cloudflare dashboard). A background-task chip carries the full diagnosis.
