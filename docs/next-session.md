...
| Metric | Value |
|---|---|
| `main` HEAD | `4af416a` (re-pull for exact; PRs #342–#349 merged this session) |
| Tests | **3053 / 264** all green |
| Build | clean · Lint **0 errors** |
| Production | deployed from `main` via Workers Builds · `https://airactionsport.com/api/health` → `{"ok":true,...}` — live Stripe + Marketing/deliverability schema + waiver receipts + accounting suite + multi-day support all deployed. **NEW: `Operation Last Light` is PUBLISHED + live** (the first/only published event; real bookings on live Stripe). |
| Migrations on remote | **0001–0076 ALL applied** — a `migrations apply` finds nothing new. (No new migrations 2026-06-27 — the focal-point + title-placement work is all `details_json` + code.) |
| Open PRs | 0 (all merged through #349) |
| Open milestone | **None active.** No milestone/roadmap items remain. **`Operation Last Light` is LIVE.** Remaining: operator activation only (Marketing send + Resend webhook + FTS flag) + finalize ticket **prices** on the still-DRAFT **Ghost Town: 18HR MILSIM** multi-day event before publishing it. CI green. |
