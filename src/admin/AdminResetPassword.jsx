...
export default function AdminResetPassword() {
  ...
  const [validity, setValidity] = useState(null); // { valid, email, reason }
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/auth/verify-reset-token/${encodeURIComponent(token)}`, { cache: 'no-store' })
      .then((r) => r.json())
  ...
  if (verifying) {
    return <div style={shell}><p style={{ color: 'var(--olive-light)' }}>Verifying link…</p></div>;
  }

  if (!validity?.valid) {
    const copy = {
      missing: 'No reset token provided.',
    ...
    return (
      <div style={shell}>
        <div style={card}>
          <div style={{ color: '#ff8a7e', fontSize: 11, fontWeight: 800, letterSpacing: 2, marginBottom: 8 }}>⚠ Link Not Valid</div>
          <h2 style={title}>Reset failed.</h2>
          <p style={{ color: 'var(--tan-light)', fontSize: 14, marginBottom: 20 }}>
            {copy[validity?.reason] || 'This link is not valid.'}
          </p>
          <Link to="/admin/forgot-password" style={{ ...btn, display: 'inline-block', textAlign: 'center', textDecoration: 'none' }}>
            ▶ Request New Link
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={shell}>
      <form onSubmit={handleSubmit} style={card}>
        <div style={{ color: 'var(--orange)', fontSize: 11, fontWeight: 800, letterSpacing: 2, marginBottom: 8 }}>■ Reset Password</div>
        <h2 style={title}>Set a new password</h2>
        <p style={{ color: 'var(--olive-light)', fontSize: 13, marginBottom: 20 }}>
          Resetting password for <strong style={{ color: 'var(--tan)' }}>{validity.email}</strong>
        </p>
