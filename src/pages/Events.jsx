            ...
            <div className={`event-card${ev.featured ? ' event-card-featured' : ''}`} key={ev.id}>
              {(ev.cardImageUrl || ev.coverImageUrl) && (
                <Link
                  ...
                  style={{ backgroundImage: `linear-gradient(180deg, rgba(0,0,0,0.0) 0%, rgba(0,0,0,${ev.cardOverlayOpacity ?? 0.65}) 100%), url(${ev.cardImageUrl || ev.coverImageUrl})`, backgroundPosition: ev.cardImagePosition || 'center' }}
