...
| `main` HEAD | **`716bd7c`** · re-pull for exact (all merged through #411). Migrations **0001–0079 applied on remote**; ⚠️ **0080 (SUA placeholder seed) is in-repo, NOT applied** — agreement uploads 409 in prod until the operator applies it. |
| Tests | **3567 / 304 passing** · reproduce CI exactly with `TZ=UTC npx vitest run` (the runner is UTC; a naive-ISO fixture is ambient-TZ-dependent) |
...
| D1 migrations on remote | **0001–0079 applied.** 0078 (`persons.legal_name` + `ein_ciphertext`) + 0079 (`additional_charge_notice` drops the dead `{{paymentLink}}`) applied 2026-07-27 and verified. ⚠️ **0080 (SUA placeholder seed, Sprint 4 #408) is in-repo and NOT yet applied** — until then agreement uploads 409. (0077 `reviews` 2026-06-28; 0076 multi-day 2026-06-26; 0074/0075 2026-06-24.) |
| Open milestone | **None active.** **Multi-day events shipped + deployed 2026-06-26** (PR #338, migration 0076; first event `Ghost Town: 18HR MILSIM` seeded as a draft #340). The **accounting-dashboard roadmap is FULLY complete (11 surfaces, 2026-06-24/25)**. **No milestone/roadmap items remain.** The **attendee-verified reviews feature is COMPLETE + DEPLOYED (2026-06-28 → 06-30)** — dormant until the first review ~2026-07-25. **`Operation Last Light` was built + PUBLISHED 2026-06-27** (the first/only live event). **The 18HR draft was rebuilt + PUBLISHED as `Operation Fire Storm` 2026-07-01** ($80, night-only) and the **reviews CAN-SPAM call was RESOLVED 2026-07-01** (transactional + suppression, #370). **The admin workflow audit is FULLY CLOSED — all four sprints** (Sprint 3: 2026-07-28 #393–#400; Sprint 4: 2026-07-28 #403–#411, migration 0080 in-repo/operator-pending). What remains is all of `docs/growth-plan-2026-07.md` and the kiosk repair-or-retire decision. ✅ The **reminder-cron timezone bug and its whole family are FIXED + DEPLOYED (2026-07-27, #391/#392)** — nothing is blocking. Two agreed follow-ups from that series: narrow event conflict windows to real start→end times, and the parked business-calendar skew (`docs/business-calendar-utc-skew.md`). Operator activation: the **Resend plan upgrade** (now evidenced — a 2026-07-26 invite run logged `considered:23 sent:13 failed:10` with `alarm:false`) + `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET`/Resend webhook, the `audit_log_fts` flag; the 2 Foxtrot cutover invoices were cancelled (#339). **CI** green. |
...
⚠️ **The list that used to sit here was months out of date** — migrations 0065–0070 were applied 2026-06-02, the marketing route-capability swap shipped the same day, and the M6 live-Stripe cutover completed 2026-06-03. **Operator-pending today is only:** ⚠️ **applying migration 0080** (the SUA placeholder seed from Sprint 4 #408 — agreement uploads 409 until it lands, and the placeholder text needs attorney review before a real renter signs; see [sua-template.md](docs/runbooks/sua-template.md)), the Resend plan upgrade, `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET` + the Resend dashboard webhook, and the `audit_log_fts` flag flip. The authoritative list is in **[docs/next-session.md](docs/next-session.md)** (⚠️ Operator-pending section), with full detail in the runbooks ([marketing-deploy.md](docs/runbooks/marketing-deploy.md), [m7-deploy.md](docs/runbooks/m7-deploy.md), [m6-operator-cutover-checklist.md](docs/m6-operator-cutover-checklist.md)).
...
3. Sanity checks:
   ...
   - `TZ=UTC npx vitest run` → **3567 passing across 304 files** (as of the 2026-07-28 admin-audit Sprint 4 close).
...
- Menu of available tracks (admin-audit Sprint 4, the growth plan, the kiosk decision, the two timezone follow-ups)
...
Production is stable at main=716bd7c (re-pull for exact), tests
3567/304 passing, 0 open PRs. Migrations 0001-0079 are applied on
remote; 0080 (the SUA placeholder seed) is in-repo and NOT applied —
agreement uploads 409 in prod until the operator applies it.

NOTHING IS BLOCKING. Pick from the work menu at the top of
docs/next-session.md — the growth plan (not started), the kiosk
repair-or-retire decision, or the two agreed follow-ups from the
timezone series.
