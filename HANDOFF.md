...
| Production health | `https://airactionsport.com/api/health` → `{"ok":true,...}` — deployed (latest Version **`fabeb661`**, 2026-07-25); **live Stripe (really cut over 2026-06-03)**; accounting suite + multi-day support + attendee-verified reviews + `/safety` + the **open-reads admin model** + the **event-day hardening** (per-event Today tiles incl. walk-in New Booking, AdminScan safeguards, review-invite tooling) all deployed. **The July 25-26 events have now RUN:** `Operation Last Light` ($60 day op) + `Operation Fire Storm` ($80 16-hr night op). Sprint-2 broken-wiring fixes deployed. |
| D1 migrations on remote | ⚠️ **0001–0077 applied; `0078_persons_tax_identity` + `0079_charge_notice_no_paylink` are IN-REPO BUT NOT APPLIED** — run `npx wrangler d1 migrations apply air-action-sports-db --remote`. (0077 `reviews` 2026-06-28; 0076 multi-day 2026-06-26; 0074/0075 2026-06-24.) |
| Open milestone | **None active.** **Multi-day events shipped + deployed 2026-06-26** (PR #338, migration 0076; first event `Ghost Town: 18HR MILSIM` seeded as a draft #340). The **accounting-dashboard roadmap is FULLY complete (11 surfaces, 2026-06-24/25)**. **No milestone/roadmap items remain.** The **attendee-verified reviews feature is COMPLETE + DEPLOYED (2026-06-28 → 06-30)** — dormant until the first review ~2026-07-25. **`Operation Last Light` was built + PUBLISHED 2026-06-27** (the first/only live event). **The 18HR draft was rebuilt + PUBLISHED as `Operation Fire Storm` 2026-07-01** ($80, night-only) and the **reviews CAN-SPAM call was RESOLVED 2026-07-01** (transactional + suppression, #370). **Admin-audit Sprints 1–2 are COMPLETE (2026-07-24/25); Sprints 3–4 remain**, as does all of `docs/growth-plan-2026-07.md`. 🔴 Ahead of both: the **reminder-cron timezone bug** (START HERE in docs/next-session.md). Operator activation: **apply migrations 0078+0079**, Marketing send (`MARKETING_POSTAL_ADDRESS` + Resend upgrade), `RESEND_WEBHOOK_SECRET`/Resend webhook, the `audit_log_fts` flag; the 2 Foxtrot cutover invoices were cancelled (#339). **CI** green. |
| Open PRs | 0 (all merged through #389 + the 2026-07-25 Sprint 2 docs sync) |
...
**Latest (2026-07-01 → 07-03): OPERATION FIRE STORM built + PUBLISHED (PRs #363–#370) — both July-25 events now take real bookings.** The `ghost-town-18hr-milsim` draft became a night-only 16-hr op (Sat 25 Jul 7:45 PM → Sun noon, single $80 "Entry" ticket `tt_NBVUMAr6EnUb` cap 350, required Russian/NATO teams picker); the `/safety` briefing page went live (footer + sitemap + the event's Required Documents); pyro wording aligned site-wide; the roster search filters by team; the **reviews CAN-SPAM call was RESOLVED** (transactional + `email_events` deliverability suppression in `reviewInvites.js`, #370). **The public visual suite was converted to a route-mocked local-serve harness** (#371 — deterministic baselines; `/api/events` had never loaded from CI, so event content finally has real visual coverage). Follow-ups #372–#375 fixed the EventDetail related-cards chunk-CSS bug + normalized event-type casing. **`main` `def6848` · 3162/277 tests · prod Version `58a2ef22` · 0 open PRs. Remaining = operator activation only** (Marketing send + Resend webhook + FTS flag — see docs/next-session.md).

**Prior (2026-06-28 → 06-30): the attendee-verified REVIEWS feature is COMPLETE + DEPLOYED** (all 7 batches, PRs #351–#356 / #358 / #359, migration `0077`) — real post-event customer ratings now populate the site + feed a legit crawler-visible `aggregateRating`, and the fabricated homepage 4.9★ was removed; **dormant in prod until the first review ~2026-07-25** (after Operation Last Light ends). Runbook `docs/runbooks/reviews-deploy.md`. `main` `4fbbf4a`, **3149/276**, migrations **0001–0077**. **Prior (2026-06-27):** `Operation Last Light` was built from the `ghost-town-iii-regular-play` draft and PUBLISHED (now the first/only live event taking real bookings) + the event image focal-point fix (`adaptEvent` never forwarded the `*ImagePosition` fields) + a per-surface cover-title placement control + home hero from the Cover image (PRs #342–#349, NO new migrations). Prior: 2026-06-26 multi-day events (#338, migration 0076) + the `Ghost Town: 18HR MILSIM` draft; 2026-06-25 finished the accounting roadmap (#330–#335); the 2 unpaid Foxtrot cutover invoices were cancelled (#339).**
...
feature (migration 0077, CAN-SPAM resolved), and BOTH July-25 events —
Operation Last Light ($60 day op) and Operation Fire Storm ($80 16-hr
night op) — are done, and those events have now RUN. The admin-workflow
audit's Sprints 1-2 are closed (Sprints 3-4 remain). Production is
stable at main=b6b26a1 (re-pull for exact), tests 3236/284 passing.

TWO THINGS TO PICK UP FIRST:
  * migrations 0078 + 0079 are IN-REPO BUT NOT APPLIED to remote D1
  * a live reminder-cron timezone bug (reminders fire ~6h early) was
    found on event day and deliberately deferred — it is the START HERE
    item in docs/next-session.md
Read these in order:
