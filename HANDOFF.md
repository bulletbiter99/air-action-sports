...
**Latest (2026-06-27): `Operation Last Light` was built from the `ghost-town-iii-regular-play` draft and PUBLISHED — it is now the first/only live event taking real bookings. The session also fixed the event image focal-point system end-to-end (the real bug: `adaptEvent` never forwarded the `*ImagePosition` fields → card/hero focal points silently used `center`) + pinned surfaces to the picker aspect ratios + added a per-surface cover-title placement control (overlay/below/hidden) + made the home hero pull from the Cover image (PRs #342–#349, NO new migrations). `main` `4af416a`, 3053/264. Prior: 2026-06-26 multi-day events (#338, migration 0076) + the `Ghost Town: 18HR MILSIM` draft; 2026-06-25 finished the accounting roadmap (#330–#335); the 2 cutover invoices were cancelled (#339).**

**M1–M7 CLOSED; M8 work menu CLEARED; the admin design-consistency sweep + #317 close-off DONE; the ACCOUNTING SUITE (#319–#328) + multi-day events + Operation Last Light all COMPLETE + deployed. No in-flight milestone branch.** Production runs on **live** Stripe keys (cutover really completed 2026-06-03 — it had been silently stuck in test mode despite the 2026-06-02 record; see memory `stripe-live-cutover-fixed-2026-06-03.md`). Waiver signing emails a confirmation receipt (2026-06-11, migration 0073). The **2026-06-24 accounting suite** (PRs #319–#328) shipped the profitability + liquidity core — income-card earned-revenue basis, deferred-revenue widget, Expenses + Budgets (migration 0074), P&L-vs-budget, per-event P&L, 13-week cash-flow forecast, and TRUE Stripe fee capture (migration 0075, nightly `runStripeFeeSync` cron, `webhooks.js` untouched). New **Finances** sidebar cluster (Expenses/Budgets/Cash Flow) + 3 new reports. See memory `accounting-dashboard-roadmap.md`. Remaining operator items: 2 outstanding cutover-remediation invoices (Kayden Case, Eduardo Ames) + the marketing/Resend/FTS activation steps in docs/next-session.md; the nightly Stripe-fee cron auto-backfills 53 paid bookings (≤2 nights, no action).
...
accounting roadmap, multi-day events, and Operation Last Light (now LIVE)
are all done + deployed; production stable at main=4af416a (re-pull for
exact), tests 3053/264 passing, migrations 0001-0076 applied, live Stripe.
Read these in order:
