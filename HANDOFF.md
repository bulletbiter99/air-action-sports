...
## ✓ Current — growth plan STARTED 2026-07-31 (8 PRs) · admin audit fully closed

**No milestone is active and nothing is blocking.** The most recent work is the **2026-07-31 growth-plan session** (PRs #418–#425, no migrations): it re-verified `docs/growth-plan-2026-07.md` against current `main` with an 11-agent workflow, ran a 12-agent dead-link sweep over 504 targets, and shipped the first slice — the archived-event OG/JSON-LD fix, Event JSON-LD un-gated, the missing `og-image.jpg`, a D1-generated sitemap, fabricated testimonials deleted, a false 50% cancellation credit removed, honest empty states, and home site cards driven by the sites API. ⭐ **Its most important finding needs no code:** AI crawlers (GPTBot, ClaudeBot, PerplexityBot, OAI-SearchBot) are **403'd at the Cloudflare edge** and Cloudflare injects a managed `robots.txt` disallowing them — `public/robots.txt` cannot fix it; it is a dashboard toggle, decided but unconfirmed. **There are also ZERO bookable events** (all 5 `published=0, past=1`). See [docs/next-session.md](docs/next-session.md), which supersedes the growth-plan document where they disagree. Before that, the admin workflow audit's **Sprint 4** (PRs #403–#411) — which **closed the audit entirely**, all four sprints. It shipped the C1 archive contract + **`sales_close_at` enforcement**, C8 pending-card lifecycle, C9 labor completion, the 1099 tax-identity editor, the B5 SUA seed (**migration 0080 — applied 2026-07-28**), the B4 recurrence API+UI, the persona dropdown, and a stale-copy sweep. The state table below is authoritative. Everything under this line is milestone history, newest first — the oldest entries describe their own moment and are not current state.

### M7 CLOSED + DEPLOYED 2026-05-31 (history)
...
| Metric | Value |
|---|---|
| Latest work | **2026-07-27 — the ENTIRE `date_iso` timezone family closed + deployed (PRs #391 + #392).** Started as the one deferred 🔴 (reminders firing ~6h early) and a blast-radius survey turned it into **~32 sites across two failure modes**: (1) *naive-string-as-instant* — `unixepoch(date_iso)`/`Date.parse` vs real epoch-ms, 6h out in MDT / 7h in MST; and (2) *UTC-"today" vs Mountain-date* — a **correct** date-portion extract compared against `date('now')`/`toISOString().slice(0,10)`, which looks safe because both sides are dates but is wrong for the six-hour band 18:00–24:00 Mountain **every day**. **`worker/lib/eventTime.js` (+ the dual-target-tested `src/utils/eventTime.js` client mirror) is now the canonical resolver** — use `eventInstantMs` / `denverDateFor` / `denverWallClockWindow` / `eventStartsWithin` / `toDenverWallClock` for any new `date_iso` consumer. **#391** fixed the reminder cron + 2 vendor sweeps (two-stage filter: wall-clock bounds in SQL, exact instant re-check in JS) and backfilled the stale gate map. **#392** fixed the rest in 5 commits: the public countdown (correct only for Mountain visitors — 2h fast on the East Coast) + `/games` rendering "Invalid Date" under every timed event; the admin "today" cluster (`/today/active` going blind from 6 PM Mountain **during an op's closing hours**, deferred revenue booking undelivered events as earned, the site archive guard); review invites (**verified in prod**: all 23 Last Light invites fired 30 min after ENDEX) + the resend gate; the conflict engine (**silent field double-booking**); and the staffing/vendor/`sales_close_at` tail. ⚠️ **Four traps worth not re-deriving:** D1's `unixepoch(x,'utc')`/`'localtime'` is a **silent no-op** (host TZ is UTC — verified against prod); a hardcoded `-06:00` is wrong ~4 months a year; **widening a LIMIT-guarded range starves due rows** (an adversarial review caught a high-severity regression the fix itself introduced); and a naive-ISO test fixture is **ambient-TZ-dependent** — run `TZ=UTC npx vitest run` to reproduce CI. Tests 3236 → **3359/287** · **migrations 0078 + 0079 APPLIED** (all 0001–0079) · prod **`2ab3f7c4`**, verified. Also: the first review moderation (1★ hidden → public aggregate **2 / 4.5★**) and the business-calendar skew parked in `docs/business-calendar-utc-skew.md`. **Prior: 2026-07-25 — admin-audit SPRINT 2 (broken wiring) + a real-schema SQL guard (PRs #383–#389).** Fixed all six Sprint 2 items: A4 CronHealth read three field names that exist nowhere (#385); A6 event-duplicate named 31 of the `events` table's 40 columns, silently dropping custom questions / site_id / every focal point (#386, now derives the carry set from the source row); B3 added a **Venue picker** + additive `formatEvent.siteId` so the M5.5 conflict engine is reachable for the first time (#387); A2 migration **0078** (`persons.legal_name` + `ein_ciphertext`) unbroke the 1099 report/CSV/cron, EIN decrypt gated on `staff.read.pii` + audited (#388); A3 migration **0079** stopped emailing a payment link to a URL that never existed (#389); plus the audit's meta-fix as a **real-schema guard** (#383) — `tests/helpers/realSchema.js` applies all migrations into in-memory SQLite and `tests/unit/schema/workerSql.test.js` `prepare()`s all 717 static SQL literals in `worker/`. ⭐ **The guard found four bugs the audit missed on its first run**, headline: `runStripeFeeSync` wrote `UPDATE bookings SET … updated_at = ?` — `bookings` has no `updated_at` — so **74 paid/refunded bookings had 0 Stripe fees captured** since 2026-06-24 and the true-fee report never had data; also the A/R aging report 500ing on `c.full_name`, `rental_items.qr_token`, and `attendees.event_id` in both check-in handlers (#384). Tests 3198 → **3236/284** · **migrations 0078 + 0079 IN-REPO, NOT YET APPLIED**. Same day: a customer booking was moved Fire Storm→Last Light with a $21.97 partial refund, and a 🔴 **live reminder-cron timezone bug** was found (reminders fire ~6h early) and deliberately deferred off event day — see docs/next-session.md START HERE + memory `reminder-cron-timezone-bug`. **Prior: 2026-07-24 → 07-25 — event-day hardening + OPEN-READS access model + growth/audit research (PRs #377–#381).** The night before the July-25-26 events: per-event Today tiles (two-event day + multi-day widget spans, #377) incl. a **walk-in "New Booking" tile** (#380, AdminNewBooking `?event=` preselect); AdminScan payment-due + wrong-event safeguards (#378); **review-invite visibility + manual resend** + the reschedule price-diff fix (#381); and the **open-reads admin access model** (#379 — every page viewable by any authenticated admin via the new `requireReadAccess` in `worker/lib/capabilities.js`; writes + field-level sensitive reads keep their gates; Settings → Charges nav added). Two in-repo roadmaps landed: `docs/growth-plan-2026-07.md` (conversion + LLM discoverability, execution not started) + `docs/admin-workflow-audit-2026-07.md` (Sprint 1 done; Sprints 2–4 remain). Tests 3162 → **3198/279** · prod `fabeb661` · no new migrations. **Prior: 2026-07-01 → 07-03 — OPERATION FIRE STORM built + PUBLISHED (PRs #363–#375).** The `ghost-town-18hr-milsim` draft became a night-only 16-hr op (Sat 25 Jul 7:45 PM → Sun noon, $80 single "Entry" ticket, required Russian/NATO teams picker) and the operator published it — **both July-25 events now take real bookings**. Same stretch: the public **`/safety`** briefing page (+ footer/sitemap + event Required-Documents link), site-wide categorical no-pyro wording, roster team-search filtering, the **reviews CAN-SPAM resolution** (transactional + `email_events` suppression, #370), the **public visual suite converted to a route-mocked harness** (#371 — deterministic baselines; event content finally visually covered), the EventDetail related-cards chunk-CSS fix (#372), and event-type casing normalization (#374/#375). `main` `def6848` · **3162/277** tests · prod `58a2ef22`. **Prior: 2026-06-28 → 06-30 — Attendee-verified REVIEWS feature ✅ COMPLETE + DEPLOYED.** Real post-event customer ratings → site + a legitimate crawler-visible `aggregateRating` (the fabricated homepage 4.9/50 **removed**). **All 7 batches merged** (PRs #351–#356, #358, #359; migration `0077` applied to prod D1): reviews schema; 03:00 invite cron; public `/api/reviews` submit/read; SSR aggregateRating on home + event pages; admin moderation (`/admin/reviews`); public `/review` star form + `/reviews` page + event/home display. **Dormant in prod until the first review (~2026-07-25, after Operation Last Light ends).** Runbook `docs/runbooks/reviews-deploy.md`; full state: `docs/next-session.md` + `docs/reviews-feature-spec.md` + memory `reviews-feature-in-progress`. (Both then-pending reviews items since RESOLVED: CAN-SPAM 2026-07-01 = transactional + suppression; the `home`-baseline concern is obsolete under the mocked visual harness.) **Prior: 2026-06-27 — OPERATION LAST LIGHT published + image/focal-point pass (PRs #342–#349).** Built the `ghost-town-iii-regular-play` draft into **Operation Last Light** (single-day 12-hr mission op, 25 Jul 2026, Ghost Town/Hiawatha UT, $60, required Russian-vs-NATO teams picker) and **published it** — now the **first/only live event, taking real bookings on live Stripe**. Then a big image pass: fixed a real bug where `adaptEvent` (`src/hooks/useEvents.js`) **never forwarded the `*ImagePosition` fields** (so event card/hero focal points silently used `center`); pinned all public surfaces to the focal-picker aspect ratios (WYSIWYG cover-crop); added a per-event **per-surface cover-title placement** control (overlay/below/hidden for the event hero + booking banner); the **home hero now pulls from the Cover (Universal Fallback)** image. **NO new migrations** (all `details_json` + data); tests 3050 → **3053**. Memory `event-image-focal-and-title-placement`. **Prior: 2026-06-26 — MULTI-DAY EVENTS shipped + deployed (PR #338) + first 2-day event seeded.** A 6-phase feature added genuine multi-day events (a structured `events.end_date_iso` span; migration **0076**): conflict detection, the event-day check-in window (+ a fixed latent timed-date NaN bug), today-active/deferred-revenue span-awareness, public date-range + per-day schedule, and an admin end-date input + per-day schedule editor. Merged after a multi-agent code review (findings folded) + all CI green; tests 3020 → **3050**. The first multi-day event **`Ghost Town: 18HR MILSIM` (25-26 July 2026)** was seeded as a **DRAFT** (#340 — since rebuilt + PUBLISHED as **Operation Fire Storm**, 2026-07-01); the 2 unpaid Foxtrot cutover invoices were **cancelled** (#339). Memory `multiday-events-feature`. **Prior: 2026-06-25 — accounting-dashboard roadmap FINISHED (#330–#336).** Shipped the last 3 roadmap items + a CI-hygiene fix: **field-rental A/R aging + DSO** (#330, Bookkeeper report), an **admin visual-baseline recapture** that turned the long-red Admin-visual CI check green again (#331), the **Owner weekly scorecard** (#332, the research's section-1 EOS Level-10 grid, judge-panel-designed), a ScorecardGrid render test (#334), and the **refund-side Stripe-fee reconciliation** (#335, completing the true-fee feature — the cron now reconciles paid **and** refunded). **No new migrations**; tests 3003 → 3020. The accounting roadmap is now **fully complete (11 surfaces)**; only operator activation remains. Prior: **2026-06-24 accounting suite (#319–#328)** — the profitability + liquidity core: income-card earned-revenue basis (#319), deferred-revenue widget (#320), Expenses + Budgets (migration 0074, #321/#322), P&L-vs-budget (#323), per-event P&L (#324), 13-week cash-flow forecast (#325/#326), TRUE Stripe fee capture (migration 0075, nightly `runStripeFeeSync` cron, `webhooks.js` untouched, #327/#328); migrations 0074–0075 applied. Prior: **2026-06-18 design-sweep close-off (#317)** — conformed the admin **home dashboard** (`AdminDashboardPersona`) + the **Sites cluster** to the shared **AdminPageHeader** house style; no schema/behavior change. Prior: the **2026-06-17 admin design-consistency sweep** — conformed every admin list/index/create-form page to the shared **AdminPageHeader** house style + chrome standardization (bordered table-box wrappers on the marketing-cluster tables + Field Rentals migrated to the shared chip-based FilterBar + EmptyState + accent button). PRs #306 (batch 1) + #308–#315, all merged + deployed; **NO schema/migration/behavior changes** (per-element token/header/chrome swaps + additive/rewritten RTL tests). Prior: the earlier 2026-06-17 session cleared both ⭐ M8 work-menu items (#297–#304); **2026-06-11** waiver-confirmation email + waiver UX (#291–#295, migration 0073); **2026-06-06** homepage reorder (#289/#290); **2026-06-03** Stripe live-cutover FIX + Volga rental content + booking-reschedule (#284) + check-in "Payment due" flag (#286). |
| `main` HEAD | **`414c556`** (merged through #426). ⚠️ **`content-truth-and-chunk-recovery` is pushed, 7 commits ahead, NOT merged, no PR** — see docs/next-session.md. Migrations **0001–0080 ALL applied on remote** — the 2026-07-31 growth-plan session added **none**. |
| Tests | **3689 / 313 passing** on branch `content-truth-and-chunk-recovery` (`main` alone is 3631 / 307) · reproduce CI exactly with `TZ=UTC npx vitest run` (the runner is UTC; a naive-ISO fixture is ambient-TZ-dependent) |
| Build | clean · Lint **0 errors** (`npx eslint src worker tests scripts`; plain `npm run lint` also walks the gitignored `static-backup/`, which CI never sees — 24 pre-existing errors there) |
| Production health | `https://airactionsport.com/api/health` → `{"ok":true,...}` — deployed + verified through #411 (re-check the current Version with `wrangler deployments list`); **live Stripe (really cut over 2026-06-03)**; accounting suite + multi-day + attendee-verified reviews + `/safety` + open-reads admin + event-day hardening + the full `date_iso` timezone fix + **all four admin-audit sprints** live. **Post-#404 archive contract:** archived (`past=1`) events render on `/games` and their public detail pages **regardless of `published`** (prod verified serving 5 archived events) and are **not bookable** (quote/checkout 409). A bare `/api/events` (no `include_past=1`) still filters to upcoming published events and can legitimately return `[]`. Reviews are **LIVE with real data**: public aggregate **3 / 4.7★** as of 2026-07-31 (re-read `GET /api/reviews/summary`; it moves). **Nothing from `content-truth-and-chunk-recovery` is deployed yet.** |
| D1 migrations on remote | **0001–0080 ALL applied.** 0080 (SUA placeholder seed, Sprint 4 #408) applied 2026-07-28 — seeded row verified incl. an independent `body_sha256` recompute; agreement uploads work, but the placeholder text needs attorney review before a real renter signs. 0078 + 0079 applied 2026-07-27 and verified. (0077 `reviews` 2026-06-28; 0076 multi-day 2026-06-26; 0074/0075 2026-06-24.) |
| Open milestone | **None active.** **Multi-day events shipped + deployed 2026-06-26** (PR #338, migration 0076; first event `Ghost Town: 18HR MILSIM` seeded as a draft #340). The **accounting-dashboard roadmap is FULLY complete (11 surfaces, 2026-06-24/25)**. **No milestone/roadmap items remain.** The **attendee-verified reviews feature is COMPLETE + DEPLOYED (2026-06-28 → 06-30)** — dormant until the first review ~2026-07-25. **`Operation Last Light` was built + PUBLISHED 2026-06-27** (the first/only live event). **The 18HR draft was rebuilt + PUBLISHED as `Operation Fire Storm` 2026-07-01** ($80, night-only) and the **reviews CAN-SPAM call was RESOLVED 2026-07-01** (transactional + suppression, #370). **The admin workflow audit is FULLY CLOSED — all four sprints** (Sprint 3: 2026-07-28 #393–#400; Sprint 4: 2026-07-28 #403–#411; migration 0080 applied 2026-07-28). What remains is all of `docs/growth-plan-2026-07.md` and the kiosk repair-or-retire decision. ✅ The **reminder-cron timezone bug and its whole family are FIXED + DEPLOYED (2026-07-27, #391/#392)**, and **both agreed follow-ups from that series CLOSED 2026-07-28 (#412–#416)**: conflict windows narrowed to real start→end times (opt-in via a timed `end_date_iso`), and the business-calendar skew closed — every financial surface windows AND buckets on the Denver calendar (measured against production first: zero figures moved). Nothing is blocking. Operator activation: the **Resend plan upgrade** (now evidenced — a 2026-07-26 invite run logged `considered:23 sent:13 failed:10` with `alarm:false`) + `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET`/Resend webhook, the `audit_log_fts` flag; the 2 Foxtrot cutover invoices were cancelled (#339). **CI** green. |
| Open PRs | 0 — but `content-truth-and-chunk-recovery` is pushed and awaiting a PR (add the `capture-baselines` label before merging). |
...
⚠️ **The list that used to sit here was months out of date** — migrations 0065–0070 were applied 2026-06-02, the marketing route-capability swap shipped the same day, and the M6 live-Stripe cutover completed 2026-06-03. **Operator-pending today (2026-07-31):** ⭐ the **Cloudflare AI-crawler toggle** (the single highest-leverage item — no code substitutes for it), a purpose-made 1200×630 `og-image.jpg` (the file now exists but is a cropped venue photo), `scripts/rename-echo-urban-blurb.sql` (the sites row still reads "Echo Urban Warehouse" — the last place that never-existed venue name survives), a decision on whether to publish an event, the **SUA attorney review** (0080 was applied 2026-07-28 and the agreement flow works, but the seeded text is a placeholder — replace per [sua-template.md](docs/runbooks/sua-template.md) before a real renter signs), the Resend plan upgrade, `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET` + the Resend dashboard webhook, and the `audit_log_fts` flag flip. The authoritative list is in **[docs/next-session.md](docs/next-session.md)** (⚠️ Operator-pending section), with full detail in the runbooks ([marketing-deploy.md](docs/runbooks/marketing-deploy.md), [m7-deploy.md](docs/runbooks/m7-deploy.md), [m6-operator-cutover-checklist.md](docs/m6-operator-cutover-checklist.md)).
...
- ~~Stripe sandbox → live cutover~~ ✅ completed 2026-06-03. (The `/admin/booking-charges/pay/:token` landing was never built; #389 stopped emailing the dead link.)
...
| Payments | Stripe direct — **LIVE since 2026-06-03. Production takes real money; never run a "test" checkout against it.** |
...
| GET | `/api/reviews/context?token=` | Attendee reviews (migration 0077). Resolve a per-booking review token → form context `{eligible, alreadyReviewed, editable, event, suggestedAuthorName, existingReview}`. Rate-limit `RL_TOKEN_LOOKUP`. 400 bad/missing · 404 unknown. Publish-independent (gated by the review's own `status`, not `events.published`). |
| POST | `/api/reviews` | Submit or edit ONE review per booking (auto-publish). Body `{token, rating 1-5, title?, comment?, authorName?, company (honeypot)}`. Rate-limit `RL_FEEDBACK`. 201 created / 200 edited (30d + ≤3 edits) / 403 ineligible / 404 unknown / 409 past-window or hidden. Output whitelisted (never booking_id/email/ip_hash/status). |
| GET | `/api/reviews?event=<id\|slug>` | One event's published feed: `{event, average, count, reviews:[…]}` (whitelisted fields only). |
| GET | `/api/reviews/summary?recent&perEvent` | `{overall:{average,count}, recent:[…]}` (+ `perEvent[]` when `=1`). Home + structured-data feed; `average:null,count:0` when no reviews. |
| GET | `/api/reviews/all` | Dedicated `/reviews` page feed: `{total, average, reviews:[…]}`. |
...
| GET | `/api/admin/reviews[?event_id=&rating=&status=&q=&limit=&offset=]` | `requireCapability('reviews.moderate')` (owner/event_director/booking_coordinator) — list + filters + `summary:{published,hidden,total,average}` + per-row `bookingFlag` (booking now refunded/cancelled). Admin-only fields (email, booking_id, ip_hash) stay inside this gated route. |
| PUT | `/api/admin/reviews/:id` | `reviews.moderate` — `{action:'hide'\|'unhide', reason?}`. hide → `status='hidden'` (+ hidden_reason/by/at, `review.hidden` audit); unhide → republish (`review.unhidden` audit). Instantly affects the public feed + both SSR aggregates. |
...
| **Booking flow cutover (Peek → internal)** | Removed the Peek widget `<script>` from `index.html`. `siteConfig.bookingLink` flipped from the Peek URL to `/booking`. 17 `<a target="_blank">` references converted to `<Link to>` across 14 files (Navbar, MobileMenu, EventCard, FloatingBookPill, PricingCard, Home ×2, About, Contact, EventDetail, Gallery, NewPlayers, Locations ×2). Events.jsx "Enquire Now" rerouted to `/contact` (sales conversation, not self-serve checkout). Stripe was still in sandbox at the time of this entry; the real-money cutover completed 2026-06-03. Plus `Booking` page got a new always-visible event banner at the top of step 1 (orange-bordered, large title, location + time + price; uses `coverImageUrl` as a darkened bg when set) so single-event mode no longer hides the event identity in a section subheading. |
...
3. ~~**Seed content for Operation Nightfall.**~~ — obsolete. Nightfall is long past; the events that actually ran (Operation Last Light + Operation Fire Storm, 25–26 Jul 2026) were built, published, sold real tickets, and are now archived. Event content is editable end-to-end via `/admin/events` → "Detail page content".
4. ~~**Invite a second admin** via `/admin/users`.~~ — the route no longer exists (`AdminUsers` was decommissioned in M5 R17; `/admin/users` redirects to `/admin/staff`). Invite from **`/admin/staff/:id` → Access tab** (needs the `staff.invite` capability). Since Sprint 4 #410 there is also a read-only **Login Accounts** list on `/admin/settings`. The underlying advice stands: don't be a single point of failure on event day.
5. ~~**Dry run:** create a test event and book a comp ticket.~~ — ⚠️ **Do not run a "test" booking against production.** Stripe is LIVE (since 2026-06-03); a card ticket is a real charge. A comp booking is $0 and technically safe, but it mints real attendees/QRs and pollutes reporting — and any leftover test rows have to be swept by hand (see `scripts/cleanup-*.sql` for the 2026-06-17 sweep that did exactly that). The operational chain has since been exercised for real across two live events.
...
- **`/admin/today` page** (M4 B12c) lives at `src/admin/AdminToday.jsx`. 3 render states based on `useTodayActive()`: loading, no-event-today (empty card pointing to /admin/events), active-event-today (header + 3 quick-action tiles to Roster / Scan / Rentals with `?event=eventId` deep-links). Sidebar Today entry (B5) renders only when `activeEventToday=true` per D09. Deep-link params: `AdminScan` and `AdminRoster` DO parse `?event=` and pre-select (this note was stale). As of `ee7ccfc` on the content-truth branch, `AdminBookings` + `AdminFeedback` (via `src/hooks/useFilterState.js`) and `AdminEvents` (`?id=`) also arrive applied.
...
- **Stripe**: ✅ **LIVE since 2026-06-03** — production takes real money
...
3. Sanity checks:
   ...
   - `curl https://airactionsport.com/api/events` → the **upcoming published** events (legitimately `[]` when none are scheduled — the July 2026 events have run and are archived). For the archive, `curl 'https://airactionsport.com/api/events?include_past=1&archive=1'` → the `past=1` events, which since #404 are served regardless of `published`.
   - `TZ=UTC npx vitest run` → **3631 passing across 307 files** (as of the 2026-07-31 growth-plan session).
...
**Latest (2026-07-27): the ENTIRE `date_iso` timezone family is FIXED + DEPLOYED (PRs #391 + #392).** `events.date_iso` stores naive America/Denver wall clock with no offset, and SQLite `unixepoch()` / JS `Date.parse()` in a Worker both read it as UTC — so ~32 sites were 6h (MDT) / 7h (MST) wrong. **`worker/lib/eventTime.js`, mirrored at `src/utils/eventTime.js`, is the canonical resolver — use it for any new `date_iso` consumer.** Fixed: reminder emails firing ~6h early (18 customers got "T-MINUS 1 HOUR" at 1:20 AM on event day), the public countdown being right only for Mountain visitors, `/games` showing "Invalid Date", `/today/active` going blind from 6 PM Mountain, deferred revenue booking undelivered events as earned, review invites firing a day early, and a silent field double-booking in the conflict engine. ⚠️ Traps: D1's `unixepoch(x,'utc')` is a **silent no-op** (host TZ is UTC); a hardcoded `-06:00` is wrong ~4 months a year; widening a LIMIT-guarded range starves due rows; a naive-ISO test fixture is ambient-TZ-dependent (**`TZ=UTC npx vitest run` reproduces CI**). (That session closed at `main` `0e64078` · 3359/287 tests · prod `2ab3f7c4`.) **CURRENT: re-pull `main` for exact HEAD · 3611/304 tests · 0 open PRs · nothing blocking** — the admin audit is fully done (#393–#411), migration 0080 is applied, and **both timezone follow-ups closed 2026-07-28 (#412–#416)**: `eventOccupancyWindow` narrows conflicts to real start→end times when a timed `end_date_iso` is set, and every financial surface windows + buckets on the DENVER calendar (`eventTime.js` business-calendar helpers + `rollupByDenverMonth`). Reviews are **LIVE with real data** (3 submitted, 1 operator-hidden → public **2 / 4.5★**). Since #404, archived (`past=1`) events are served publicly regardless of `published`; a bare `/api/events` still lists only upcoming published events and can legitimately be `[]`. Work menu: the **growth plan** (not started) and the kiosk repair-or-retire decision. Operator activation: the SUA attorney review, the **Resend plan upgrade** (evidenced — a 2026-07-26 invite run logged `considered:23 sent:13 failed:10`), `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET` + webhook, `audit_log_fts` flag.

**Prior (2026-07-01 → 07-03): OPERATION FIRE STORM built + PUBLISHED (PRs #363–#375)** — both July-25 events took real bookings; `/safety` went live; the public visual suite became a route-mocked local-serve harness (#371).

**Prior (2026-06-28 → 06-30): the attendee-verified REVIEWS feature is COMPLETE + DEPLOYED** (all 7 batches, PRs #351–#356 / #358 / #359, migration `0077`) — real post-event customer ratings now populate the site + feed a legit crawler-visible `aggregateRating`, and the fabricated homepage 4.9★ was removed; ~~dormant~~ **now LIVE with real reviews** (first 3 arrived 2026-07-25/26). Runbook `docs/runbooks/reviews-deploy.md`. `main` `4fbbf4a`, **3149/276**, migrations **0001–0077**. **Prior (2026-06-27):** `Operation Last Light` was built from the `ghost-town-iii-regular-play` draft and PUBLISHED (now the first/only live event taking real bookings) + the event image focal-point fix (`adaptEvent` never forwarded the `*ImagePosition` fields) + a per-surface cover-title placement control + home hero from the Cover image (PRs #342–#349, NO new migrations). Prior: 2026-06-26 multi-day events (#338, migration 0076) + the `Ghost Town: 18HR MILSIM` draft; 2026-06-25 finished the accounting roadmap (#330–#335); the 2 unpaid Foxtrot cutover invoices were cancelled (#339).**

**M1–M7 CLOSED; M8 work menu CLEARED; the admin design-consistency sweep + #317 close-off DONE; the ACCOUNTING SUITE (#319–#328) + multi-day events + Operation Last Light all COMPLETE + deployed. No in-flight milestone branch.** Production runs on **live** Stripe keys (cutover really completed 2026-06-03 — it had been silently stuck in test mode despite the 2026-06-02 record; see memory `stripe-live-cutover-fixed-2026-06-03.md`). Waiver signing emails a confirmation receipt (2026-06-11, migration 0073). The **2026-06-24 accounting suite** (PRs #319–#328) shipped the profitability + liquidity core — income-card earned-revenue basis, deferred-revenue widget, Expenses + Budgets (migration 0074), P&L-vs-budget, per-event P&L, 13-week cash-flow forecast, and TRUE Stripe fee capture (migration 0075, nightly `runStripeFeeSync` cron, `webhooks.js` untouched). New **Finances** sidebar cluster (Expenses/Budgets/Cash Flow) + 3 new reports. See memory `accounting-dashboard-roadmap.md`. Remaining operator items: the marketing/Resend/FTS activation steps in docs/next-session.md (the reviews CAN-SPAM classification was RESOLVED 2026-07-01 — transactional + suppression, #370). (The 2 outstanding cutover-remediation invoices — Kayden Case, Eduardo Ames — were **cancelled** 2026-06-25/26 via #339; the nightly Stripe-fee cron backfill is long done.)

**Use [docs/next-session.md](docs/next-session.md) — it is THE entry point.** It has:
- Current production state (test counts, deploy version, etc.)
- Menu of available tracks (the growth plan, the kiosk decision — the audit and both timezone follow-ups are done)
...
closed; M8's work menu, the admin design-consistency sweep, the
accounting roadmap, multi-day events, the attendee-verified reviews
feature (now LIVE with real reviews), and BOTH July-25 events —
Operation Last Light ($60 day op) and Operation Fire Storm ($80 16-hr
night op) — are done, and those events have RUN. The admin-workflow
audit is FULLY CLOSED — all four sprints; Sprint 3 shipped 2026-07-28
as PRs #393-#400 (no migrations) and Sprint 4 the same day as PRs
#403-#411 (archive contract + sales_close_at enforcement, pending-card
lifecycle, labor completion, the 1099 tax-identity editor, the SUA
seed, recurrence API+UI, the persona dropdown). The entire date_iso
timezone family was fixed + deployed 2026-07-27 (PRs #391 + #392),
and both of its follow-ups closed 2026-07-28 in PRs #412-#416
(conflict windows narrowed to real start-end times; the business
calendar moved onto Denver windows AND buckets, with a measured
zero-impact report against production). Production is stable at main
(re-pull for exact), tests 3611/304 passing, 0 open PRs. Migrations
0001-0080 are ALL applied on remote (0080 applied 2026-07-28; the
seeded SUA is a placeholder needing attorney review before a real
renter signs).

NOTHING IS BLOCKING. Pick from the work menu at the top of
docs/next-session.md — the growth plan (not started) or the kiosk
repair-or-retire decision.

BEFORE TOUCHING ANY DATE LOGIC: worker/lib/eventTime.js (mirrored at
src/utils/eventTime.js) is the canonical resolver. events.date_iso is
naive America/Denver wall clock — SQLite unixepoch() and JS Date.parse()
in a Worker both read it as UTC. D1's unixepoch(x,'utc') is a SILENT
no-op. Reproduce CI with `TZ=UTC npx vitest run`, not plain vitest.
Read these in order:
  ...
  3. CLAUDE.md — entry-point rules: the do-not-touch list (mirrored
     ...
     **Carry-forward: D1 quirks** subsection (BEGIN/COMMIT keywords
     ...
     session sections at the TOP of the file (newest first — the
     2026-07-28 Sprint 4 + Sprint 3 closes and the 2026-07-27
     timezone family).
  4. docs/admin-workflow-audit-2026-07.md (FULLY CLOSED 2026-07-28,
     all four sprints, PRs #383-#411 — read as history)
     and docs/growth-plan-2026-07.md (not started). NOTE: the M6
     cutover docs are HISTORY — the live-Stripe cutover completed
     2026-06-03; read them only for background.
...
For anything not covered above, read docs/next-session.md — it is kept
current every session. Do not rely on the historical block below this
code fence.
```

---

## Historical — M4-era resumption prompt (2026-05-07). DO NOT COPY.

> ⚠️ Preserved for provenance only. This block was written on **2026-05-07** and was never
> updated; the live prompt above superseded it. It describes M4 as in flight, Stripe as
> sandbox, ~918 tests as the gate, and a pre-launch checklist that has since been completed.
> Every operational instruction in it is stale. The CLAUDE.md milestone tables cross-reference
> this material, which is why it is kept rather than deleted.
