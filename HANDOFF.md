...
## ✓ Current — admin-audit SPRINT 3 CLOSED + DEPLOYED 2026-07-28

**No milestone is active and nothing is blocking.** The most recent work is the admin workflow audit's **Sprint 3** (PRs #393–#400, no migrations); the state table below is authoritative. Everything under this line is milestone history, newest first — the oldest entries describe their own moment and are not current state.
...
| `main` HEAD | **`6f2a2c2`** + docs sync · re-pull for exact (all merged through #401). Migrations **0001–0079 ALL APPLIED**; Sprint 3 added none. |
| Tests | **3494 / 296 passing** · reproduce CI exactly with `TZ=UTC npx vitest run` (the runner is UTC; a naive-ISO fixture is ambient-TZ-dependent) |
...
⚠️ **The list that used to sit here was months out of date** — migrations 0065–0070 were applied 2026-06-02, the marketing route-capability swap shipped the same day, and the M6 live-Stripe cutover completed 2026-06-03. **Operator-pending today is only:** the Resend plan upgrade, `MARKETING_POSTAL_ADDRESS`, `RESEND_WEBHOOK_SECRET` + the Resend dashboard webhook, and the `audit_log_fts` flag flip. The authoritative list is in **[docs/next-session.md](docs/next-session.md)** (⚠️ Operator-pending section), with full detail in the runbooks ([marketing-deploy.md](docs/runbooks/marketing-deploy.md), [m7-deploy.md](docs/runbooks/m7-deploy.md), [m6-operator-cutover-checklist.md](docs/m6-operator-cutover-checklist.md)).
...
3. Sanity checks:
   ...
   - `TZ=UTC npx vitest run` → **3494 passing across 296 files** (vitest unit suite as of the 2026-06-24 accounting-suite close).
...
audit's Sprints 1-3 are closed (Sprint 4 remains) — Sprint 3 shipped
2026-07-28 as PRs #393-#400 with no migrations. The entire date_iso
timezone family was fixed + deployed 2026-07-27 (PRs #391 + #392).
Production is stable at main=6f2a2c2 (re-pull for exact), tests
3494/296 passing, migrations 0001-0079 ALL applied, 0 open PRs.

NOTHING IS BLOCKING. Pick from the work menu at the top of
docs/next-session.md — admin-audit Sprint 4, the growth plan (not
started), the kiosk repair-or-retire decision, or the two agreed
follow-ups from the timezone series.
...
Read these in order:
  ...
  3. CLAUDE.md — entry-point rules: the do-not-touch list (mirrored
     ...
     **Carry-forward: D1 quirks** subsection (BEGIN/COMMIT keywords
     ...
     2026-07-28 Sprint 3 close and the 2026-07-27 timezone family).
  4. docs/admin-workflow-audit-2026-07.md (Sprint 4 is what remains)
