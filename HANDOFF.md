...
| Open milestone | **None active.** **Multi-day events shipped + deployed 2026-06-26** (PR #338, migration 0076; first event `Ghost Town: 18HR MILSIM` seeded as a draft #340). The **accounting-dashboard roadmap is FULLY complete (11 surfaces, 2026-06-24/25)**. **No milestone/roadmap items remain.** Remaining is operator activation only (Marketing send + Resend webhook + FTS flag) + finalize prices on the seeded multi-day draft before publishing; the 2 Foxtrot cutover invoices were cancelled (#339). **CI** green. |
| Open PRs | 0 (all merged through #340) |
...
closed; M8's work menu, the admin design-consistency sweep, and the
2026-06-24 accounting suite (#319-#328) are all done + deployed;
production stable at main=bc8614c (re-pull for exact), tests 3003/259
passing, migrations 0001-0075 applied, live Stripe. Read these in order:
