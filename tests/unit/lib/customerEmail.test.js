// Tests for worker/lib/customerEmail.js — decision register #32.
// Mirrored by tests/unit/utils/customerEmail.test.js (client target).
// Both must pass identically — proves byte-identical normalization.
...
import {
    ...
    isGmailLike,
    emailsMatch,
} from '../../../worker/lib/customerEmail.js';

describe('normalizeEmail — Gmail family', () => {
    ...
    it('combines all Gmail rules: dots + plus + googlemail + case', () => {
        // Sarah's full M3-batch-1 fixture set must all collapse to one form.
...
describe('normalizeEmail — non-Gmail providers', () => {
    it('preserves dots (different addresses)', () => {
        // Decision register #32 — different humans on Yahoo/non-Gmail
...
describe('normalizeEmail — case + whitespace + Unicode', () => {
    ...
    it('NFC-normalizes Unicode (precomposed vs decomposed)', () => {
        ...
        const precomposed = 'café@example.com';      // café (one codepoint)
        const decomposed = 'café@example.com';      // café (two codepoints)
        expect(normalizeEmail(precomposed)).toBe(normalizeEmail(decomposed));
    });
});

describe('normalizeEmail — invalid inputs', () => {
    ...
    it('returns null when entire local part is plus-alias', () => {
        // After +-strip, local would be empty
...
describe('isGmailLike', () => {
    ...
    it('returns false for Gmail subdomains (subdomain ≠ gmail.com)', () => {
...
describe('extractBaseEmail', () => {
    it('returns the canonical normalized form (alias for normalizeEmail)', () => {
        // Same logic; same return. Sanity-check on a few representative inputs.
